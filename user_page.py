...
class UsersPage:
    def __init__(self, repo: PostgresConnector):
        ...
        # When selection changes, update buttons (NiceGUI passes an event arg; our handler accepts e=None)
        self.table.on('selection', self.update_buttons)

        # Bind search box to table filter
    ...
    def refresh(self):
        ...
        # Clear selection if it exists (quasar table creates .selected lazily)
            ...
            with ui.row().classes('justify-end gap-2 mt-2'):
                ...
                def delete():
                    try:
                        ...
                        ui.notify('User undeleted' if deleted else 'User soft-deleted', type='positive')
                        dialog.close()
                        self.refresh()
                    except Exception as e:
                        ui.notify(f'Database error: {e}', type='negative')
